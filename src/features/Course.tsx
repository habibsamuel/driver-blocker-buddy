  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Calcul automatique du tarif</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2"><Label>Durée (min)</Label><Input type="number" value={duration} onChange={(e)=>setDuration(e.target.value)} /></div>
              <div className="space-y-2"><Label>Attente (min)</Label><Input type="number" value={wait} onChange={(e)=>setWait(e.target.value)} /></div>
              <div className="space-y-2"><Label>Heure (0-23)</Label><Input type="number" value={hour} onChange={(e)=>setHour(e.target.value)} /></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Row label={`Multiplicateur horaire`} value={`×${fare.peakMultiplier.toFixed(2)}`} />
            <div className="border-t pt-3 mt-3">
              <div className="flex items-baseline justify-between">
                <span className="text-lg font-semibold">TOTAL</span>
                <span className="text-3xl font-bold text-primary">{fare.total} XAF</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Min {settings.minFare} · Max {settings.maxFare} XAF · Paiement post-course</p>
            </div>
            <Button className="w-full" size="lg" onClick={handleSubmit}>Lancer la course</Button>
          </CardContent>
        </Card>
      </div>
    </div>
